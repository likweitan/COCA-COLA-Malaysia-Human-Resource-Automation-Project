...
<head>
	...
	<?php
		...
		$sql = "SELECT * FROM passwordresettemp LEFT JOIN account ON passwordresettemp.AccountNo=account.AccountNo WHERE account.AccountNo='".$employeeno."' and ExpiryDate >= Current_Timestamp() ORDER BY ExpiryDate DESC LIMIT 1";
	...
	$expirydate = $row['ExpiryDate'];
		...
		//----------------------------
		$sql = "SELECT *,DATEDIFF(Mod_Timestamp,CURRENT_TIMESTAMP()) AS DayDiff FROM point WHERE AccountNo ='".$employeeno."' ORDER BY Mod_Timestamp DESC LIMIT 1";
		$result = mysqli_query($con, $sql);
		$row = mysqli_fetch_array($result);	
		$count = mysqli_num_rows($result);
		if($count == 0 || $row['DayDiff']!=0){
			$random = rand(1,10);
			$sql = "INSERT INTO point (AccountNo,Amount) VALUES ('$employeeno','$random')";
			$result = mysqli_query($con, $sql);
		}
		$sql = "SELECT SUM(Amount) AS Total FROM point WHERE AccountNo='".$employeeno."'";
	$result = mysqli_query($con, $sql);
		$row = mysqli_fetch_array($result);	
	$point_total=$row['Total'];
	...
	<div class="dashboard-main-wrapper">
								...
								<li>
									...
									<div class="notification-list">
										<div class="list-group">
											<?php 
											$sql = "SELECT *,TIMESTAMPDIFF(SECOND,point.Mod_Timestamp,CURRENT_TIMESTAMP()) AS SecondDiff,
							TIMESTAMPDIFF(MINUTE,point.Mod_Timestamp,CURRENT_TIMESTAMP()) AS MinuteDiff,
							TIMESTAMPDIFF(HOUR,point.Mod_Timestamp,CURRENT_TIMESTAMP()) AS HourDiff,
							TIMESTAMPDIFF(DAY,point.Mod_Timestamp,CURRENT_TIMESTAMP()) AS DayDiff,
							TIMESTAMPDIFF(MONTH,point.Mod_Timestamp,CURRENT_TIMESTAMP()) AS MonthDiff
							FROM point WHERE AccountNo='".$employeeno."' ORDER BY point.Mod_Timestamp DESC";
		...
		while($row = mysqli_fetch_array($query)){
			...
			$point = $row['Amount'];
				...
				echo $name;
			echo '</span>have received ';echo $point;echo ' points.';
				...
				<nav class="navbar navbar-expand-lg navbar-light">
					...
					<div class="collapse navbar-collapse" id="navbarNav">
						<ul class="navbar-nav flex-column">
							...
							<li class="nav-item ">
								<a class="nav-link deactive" href="form_user.php"><i class="fas fa-file"></i>Document</a>
							</li>
							<li class="nav-item ">
...
if (($hour == 'AM')) {
	echo 'Good morning, ';
					...
					<div class="row">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="container">
								<div class="text-center"><a href="login.php"><img class="logo-img" src="assets/images/logo.png" alt="logo" width=100></a></div>
								<h1 class="text-center">Human Resource</h1>
					...
					<div class="row">
						<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="container">
								...
								<h1 class="text-center counter"><?=$point_total?></h1>
